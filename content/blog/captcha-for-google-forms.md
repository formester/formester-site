...
<p>The weakness is that the question never changes. A real CAPTCHA generates a fresh challenge per visitor and verifies it server-side. This is one fixed question sitting in the page, so anyone targeting your form solves it once and hardcodes the answer. It also taxes every honest respondent with arithmetic, to filter out scripts an invisible check would have caught without anyone noticing.</p>
<p>Worth it as a five-minute speed bump on a low-stakes form. Not worth mistaking for protection.</p>
...
<p>Not the widget. Add-ons run against the form and its responses through Google's APIs, and none of them can inject a challenge into markup they do not control. What some offer instead is scoring or filtering after submission.</p>
<p>Read that carefully before you install one, because the sequence matters. The bot still submits. The junk still lands in your responses and your linked Sheet. The add-on flags it afterwards, which cleans your view without reducing what you collected, and it holds standing access to every response in the process.</p>
...
<details><summary>How do I add reCAPTCHA to a Google Form?</summary><p>You cannot. reCAPTCHA is embedded with a site key in your own page's HTML, and Google Forms gives you no access to the form's markup. Embedding the form in your site does not help either, because the iframe belongs to Google.</p></details>
