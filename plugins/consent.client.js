...
function setCookie(name, value, days) {
  ...
  const domain =
    host && host.endsWith('formester.com') ? ';domain=.formester.com' : ''
...
async function enableGtm(gtmId) {
  ...
  // Inject GTM
  const gtmSrc = `https://www.googletagmanager.com/gtm.js?id=${encodeURIComponent(
    gtmId
  )}`
...
export default defineNuxtPlugin((nuxtApp) => {
  ...
  async function enableTracking() {
    ...
    try {
      await Promise.all([enableGtm(gtmId), enableClarity(clarityId)])
