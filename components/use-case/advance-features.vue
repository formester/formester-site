<template>
  <div class="feature__hero">
    <div class="container py-5 my-3">
      ...
      >
        <span
          v-for="(button, index) in features[0]"
          :key="index"
          class="toggler__button"
          :class="activeIndex === index ? 'active' : ''"
          @click="setActive(index)"
          >{{ features[0][index].heading }}</span
        >
      </div>
      <div
      ...
      >
        <span
          v-for="(button, index) in features[1]"
          :key="5 + index"
          class="toggler__button"
          :class="activeIndex === 5 + index ? 'active' : ''"
          @click="setActive(5 + index)"
          >{{ features[1][index].heading }}</span
        >
      </div>
      <div class="row py-3">
        <div
        ...
        >
          <h3 class="feature__heading">
            {{ computedFeatures[activeIndex].heading }}
          </h3>
          <p class="feature__description mt-3">
            {{ computedFeatures[activeIndex].description }}
          </p>
        </div>
        <div
        ...
        >
          <nuxt-img
            :src="`/features/${computedFeatures[activeIndex].img}.svg`"
            alt="Hero-Image"
...
export default {
  ...
  mounted() {
    console.log(this.computedFeatures)
  },
}
</script>
<style scoped>
